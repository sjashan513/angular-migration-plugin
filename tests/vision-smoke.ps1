...
@{ username = 'vision-user'; password = 'vision-password' } |
    ConvertTo-Json | Set-Content (Join-Path $tempRoot 'auth.json') -Encoding UTF8

Push-Location $tempRoot
try {
    $server = Start-Process node -ArgumentList @($serverPath, $port, 'vision-user', 'vision-password') -PassThru -WindowStyle Hidden
    $authHeader = @{ Authorization = 'Basic ' + [Convert]::ToBase64String([Text.Encoding]::ASCII.GetBytes('vision-user:vision-password')) }
    $ready = $false
    for ($attempt = 0; $attempt -lt 30; $attempt++) {
        try {
            $response = Invoke-WebRequest "http://127.0.0.1:$port/baseline/" -Headers $authHeader -UseBasicParsing -TimeoutSec 1
    ...
    $baseline = (& powershell -NoProfile -File $scriptPath -Command vision-run -VisionMode baseline -ManifestPath 'routes.json' -RuntimeUrl "http://127.0.0.1:$port/baseline/" -OutputDir '.angular-migration\vision\test' -AuthFile 'auth.json') | ConvertFrom-Json
    ...
    $comparison = (& powershell -NoProfile -File $scriptPath -Command vision-run -VisionMode compare -ManifestPath 'routes.json' -RuntimeUrl "http://127.0.0.1:$port/candidate/" -OutputDir '.angular-migration\vision\test' -PublishDir 'published' -AuthFile 'auth.json') | ConvertFrom-Json
    ...
    Assert ((Get-ChildItem '.angular-migration\vision\test\baseline' -Recurse -Filter 'page.png').Count -eq 2) 'conserva todas las capturas baseline'
    Assert ((Get-ChildItem '.angular-migration\vision\test\candidate' -Recurse -Filter 'page.png').Count -eq 2) 'conserva todas las capturas candidatas'
}
finally {
